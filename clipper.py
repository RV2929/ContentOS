...
def _cut_trim(video_path: str, start: float, end: float, output_path: str) -> str:
    """
    Cut [start, end] from video_path into output_path with no crop, captions,
    watermark, or re-encoding of the visual content — used for the long-form
    pass, which must stay in the source's native aspect ratio/resolution.

    Tries a fast stream-copy trim first (lossless, no quality loss); falls
    back to a re-encode if the copy produces a bad/empty file, e.g. because
    the seek point doesn't land cleanly on a keyframe for this container.
    """
    duration = round(end - start, 3)
    if duration <= 0:
    ...
    if result.returncode != 0 or not os.path.exists(output_path) or os.path.getsize(output_path) == 0:
        ...
        if result.returncode != 0:
            raise RuntimeError(
                f"FFmpeg failed cutting long-form segment {start}–{end}s:\n{result.stderr[-1000:]}"
            )
